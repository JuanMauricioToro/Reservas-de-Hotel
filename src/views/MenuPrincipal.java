...
public class MenuPrincipal extends JFrame {
	...
	public MenuPrincipal() {
		...
		JLabel lblCopyR = new JLabel("Desarrollado por Mauricio Toro © 2023");
	...
	//Elimina registros de reservas huerfanos en la base de datos

	public void eliminarRegistrosHuerfanos() {
		HuespedDAO h = new HuespedDAO(new ConnectionFactory().recuperarConexion());
		ReservaDAO r = new ReservaDAO(new ConnectionFactory().recuperarConexion());

		var hues = h.mostrarHuespedes("");
		var reser = r.mostrarReservas("");
		List<String> lisIdRHues = new ArrayList<>();
		List<String> lisIdR = new ArrayList<>();
		List<String> huerfanos = new ArrayList<>();

		for (Huesped hu : hues) {
			lisIdRHues.add(hu.getIdReserva());
		}
		for (Reserva re : reser) {
			lisIdR.add(re.getId());
		}

		for (int i = 0; i < lisIdR.size(); i++) {

			if (!lisIdRHues.contains(lisIdR.get(i))) {
				
				//lista de id de reservas que se registró sin crear un huesped relacionado
				huerfanos.add(lisIdR.get(i));
			}
		}
		...
		for (int i = 0; i < huerfanos.size(); i++) {
			r.eliminar(huerfanos.get(i));
		}

		
		System.out.println(huerfanos);
	}
}
